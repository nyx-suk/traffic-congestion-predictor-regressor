...
def load_model():
    try:
        ...
        print("Model or scaler file not found:", e)
        return None, None
...
def main():
    st.title('Machine Learning based Traffic Congestion Predictor')
    ...
    temperature = st.sidebar.slider('Temperature (°C)', 20, 45, 25)
    ...
    if prediction >= 6:
        ...
        best_times = sorted((h, f) for h, f in zip(hours, forecasts) if f < 5)

        if best_times:
            st.write('Recommended travel times:')
            for hour, _ in best_times[:3]:
                st.write(f"⁘ {hour:02d}:00 (Predicted congestion: {forecasts[hour]:.1f})")
